...
export default function FleetJobsPage() {
  ...
  const [selectedJobForTech, setSelectedJobForTech] = useState<Job | null>(null);
  ...
  const assignTechnicianToJob = async (technicianId: number, technicianName: string) => {
  ...
  const filteredTechnicians = technicians.filter((tech) => {
    ...
    const jobLocationTerms = selectedJobForTech.location.toLowerCase().split(/[\s,]+/);
      ...
      <div className="flex-1 space-y-4 p-4 pt-6">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Breakdown Request">Breakdown Request</SelectItem>
        ...
        <Dialog open={isTechDialogOpen} onOpenChange={setIsTechDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Assign Technician to Job: {selectedJobForTech?.job_id}</DialogTitle>
              <DialogDescription>Search and select a technician based on location or rating.</DialogDescription>
            </DialogHeader>

            <div className="mb-3">
              <Input
                placeholder="Filter by location..."
                value={searchTechnicianLocation}
                onChange={(e) => setSearchTechnicianLocation(e.target.value)}
                autoFocus
              />
            </div>
            ...
            <div className="max-h-[300px] overflow-y-auto space-y-2">
              {filteredTechnicians.filter((tech) => {
                const techLocationTerms = tech.location.toLowerCase().split(/[\s,]+/);
                const searchTerms = searchTechnicianLocation.toLowerCase().split(/[\s,]+/);
                // Return true if any job location keyword is included in tech location terms
                return searchTerms.some(term => techLocationTerms.includes(term));
              })
                .map((tech) => (
                  <div
                    key={tech.id}
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        <span className="text-gray-600">Specialties:</span> {tech.specialties?.length > 0 ? tech.specialties.join(', ') : 'None'}
                      </div>
                    </div>
                    <Button
                      size="sm"
