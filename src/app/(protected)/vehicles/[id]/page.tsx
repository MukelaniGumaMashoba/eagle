    ...
    if (!vehicle) {
        return (
            <div className="flex flex-col items-center justify-center h-screen">
                <p className="text-lg font-semibold text-gray-600">Vehicle not found</p>
            </div>
        );
    }
        ...
        >
            <div className="max-w-6xl mx-auto">
                ...
                <Card className="p-6 shadow-xl">
                    <CardHeader>
                        <CardTitle className="text-xl font-semibold">Vehicle Information</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        ...
                        >
                            ...
                            <div className="flex-1 grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="font-semibold">{vehicle.vin_number ?? "N/A"}</p>
                                </div>
                                <div>
                                    <p className="text-gray-500">Engine Number</p>
                                    <p className="font-semibold">{vehicle.engine_number ?? "N/A"}</p>
                                </div>
                                <div>
                        ...
                        >
                            ...
                            <Detail label="Priority" value={<Badge>{vehicle.vehicle_priority}</Badge>} />
                            <Detail label="Purchase Price" value={`R ${vehicle.purchase_price ?? "N/A"}`} />
                            <Detail label="Retail Price" value={`R ${vehicle.retail_price ?? "N/A"}`} />
                            <Detail label="Tank Capacity" value={`${vehicle.tank_capacity ?? "N/A"} L`} />
                            <Detail label="Take On KM" value={`${vehicle.take_on_kilometers}`} />
                            <Detail label="Boarding Hours" value={vehicle.boarding_km_hours ?? "N/A"} />
                            <Detail label="Cost Centres" value={vehicle.cost_centres ?? "N/A"} />
                        </motion.div>
                        ...
                        >
                            <Detail label="Registration Date" value={vehicle.registration_date ?? "N/A"} />
                            <Detail label="License Expiry" value={vehicle.license_expiry_date ?? "N/A"} />
                            <Detail label="Expected Boarding Date" value={vehicle.expected_boarding_date ?? "N/A"} />
                        </motion.div>
                    </CardContent>
                </Card>
            </div>
